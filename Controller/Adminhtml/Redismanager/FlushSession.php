 ...
 */
class FlushSession extends \Magento\Backend\App\Action
{
    /**
    ...
    public function execute()
    {
        if ($this->getScopeConfig('redismanager/setting/syncflush')) {
            ...
            if (!empty($flushThis['database']) && $flushThis['database'] == 0 || $flushThis['database'] != "") {
                ...
                if ($output = "OK") {
                    $this->messageManager->addSuccessMessage('The Session and Magento Cache has been flushed');
                } else {
                    $this->messageManager->addErrorMessage('The Session and Magento Cache not flushed');
                }
            } else {
                $this->messageManager->addErrorMessage('The Session and Magento Cache not flushed');
            }
            ...
            if (!empty($flushThis['database']) && $flushThis['database'] == 0 || $flushThis['database'] != "") {
                ...
                exec($commands, $output, $return);
                if ($output = "OK") {
                    $this->messageManager->addSuccessMessage('The Session has been flushed.');
                } else {
                    $this->messageManager->addErrorMessage('The Session has been flushed');
                }
            } else {
                $this->messageManager->addErrorMessage('The Session has been flushed');
            }
